...
def lambda_handler(event, context):
	...
	for instance in instances:
		...
		for image in images:
			...
			if image.description.startswith('Lambda Snapshot of ' + instance['InstanceId']):

				# print "FOUND IMAGE " + image.id + " FOR INSTANCE " + instance['InstanceId']

				# Count this image's occurrence
				...
				# Uncomment the fake time for testing purposes
				# today_date = time.strptime('2022-11-01', '%Y-%m-%d')
				# pprint.pprint(today_date)
		...
		if backupSuccess:
			...
			for image in imagesList:
				print("will deregister image %s" % image)
